	...
	for (auto i = 0;i < 8;++i)
		for (auto j = 0;j < 8;++j)
			if (checkCollisionBetweenTwoCars(std::make_pair(car1->getCollisionHitbox(static_cast<Car::hitboxPosition>(i)), car2->getCollisionHitbox(static_cast<Car::hitboxPosition>(j))),
				std::make_pair(&car1->getBoolIsCollision(static_cast<Car::collisionSide>(i)), &car2->getBoolIsCollision(static_cast<Car::collisionSide>(j)))))
...
bool carCollisionWithCar::checkCollisionBetweenTwoCars(std::pair<const std::vector<sf::CircleShape*>, const std::vector<sf::CircleShape*>>hitbox, std::pair<bool*, bool*> isCollision)
{

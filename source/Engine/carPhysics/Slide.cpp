...
void Slide::setOverSteer(const int & drivingStatus)
{
	...
	if (car->getSpeed() > 1 && !car->getStateMoving() &&
		...
		(car->getTypeOfDrive() ? 
		(isTireOnGrass(car->getTires()->getTiresPos(2)) && isTireOnGrass(car->getTires()->getTiresPos(3))) 
			: (isTireOnGrass(car->getTires()->getTiresPos(0)) && isTireOnGrass(car->getTires()->getTiresPos(1))))) ) 
	{
